...
// TODO(johnrjj) - Thread this
import { orderBook } from "./sample-orderbook-data";

const bgColor = "#1b1b25";
const lightBlue = "#5582D6";

interface MinimumDatum {
  price: number;
  amount: number;
}

// accessors
const getPrice = (d: MinimumDatum) => d.price;
const getVolume = (d: MinimumDatum) => d.amount;
...
const OrderbookChart = withTooltip(
  ({
    width,
    height,
    margin = { top: 0, right: 0, bottom: 40, left: 40 },
  }: OrderbookChartProps) => {
    const askDataUnstepped: MinimumDatum[] = orderBook.asks.sort((a, b) =>
      a.price > b.price ? 1 : -1
    );
    const bidDataUnstepped: MinimumDatum[] = orderBook.bids.sort((a, b) =>
      a.price > b.price ? 1 : -1
    );

    const addSteps = (list: MinimumDatum[]) => {
      return list.reduceRight(
        (acc, d): { prev: MinimumDatum; list: MinimumDatum[] } => {
          let curr: Array<MinimumDatum> = [
        ...
        {} as { prev: MinimumDatum; list: MinimumDatum[] }
      );
    };
    ...
    const getMinMax = (
      func: (d: MinimumDatum) => number | null | undefined,
      axisBuffer: number
    ) => {
    ...
    const acX = useCallback(
      (d: MinimumDatum) => {
        return priceScale(getPrice(d))!;
      },
      [priceScale]
    );
    const acY = useCallback(
      (d: MinimumDatum) => {
        return volumeScale(getVolume(d))!;
      },
      [volumeScale]
    );
    return (
