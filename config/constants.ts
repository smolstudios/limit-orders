// NOTE(johnrjj) - Cannot be e.g. 3 * 10 -- needs to be static number to work with server components
export const DEFAULT_CACHE_TIME_FOR_ORDERS_IN_SECONDS = 30
...
export const getNativeTokenAddress = (_chainId: string | number): `0x${string}` => {
  return NATIVE_TOKEN_ADDRESS
}

export const isNativeToken = (tokenAddress: string, _chainId: string | number) => {
  return tokenAddress.toLowerCase() === NATIVE_TOKEN_ADDRESS
}

export const monkeyPatchNativeTokenWithWrappedTokenIfNativeToken = (
...
export const getZeroExApiRootUrl = (chainId: number) => {
...
export const GOERLI_TOKENS_BY_SYMBOL: Record<string, Token> = GOERLI_TOKENS.reduce((acc, curr) => {
  const { symbol } = curr
  return {
    ...acc,
...
const MAINNET_TOKENS_BY_SYMBOL: Record<string, Token> = MAINNET_TOKENS.reduce((acc, curr) => {
  const { symbol } = curr
  return {
    ...acc,
    [symbol.toLowerCase()]: curr,
